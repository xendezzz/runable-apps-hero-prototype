(() => {
  ...
  source.setAttribute('aria-label','Describe your app idea');
  ...
  form.hidden = true;
  form.setAttribute('aria-label', 'Describe your app');
  form.innerHTML = `<button type="button" class="builder-close" aria-label="Close app idea input">×</button><textarea id="website-idea" aria-label="Your app idea" placeholder="Describe the app you want to build…" maxlength="2000" rows="2" required></textarea><div class="builder-actions"><span class="builder-hint" aria-hidden="true">press enter <kbd>↵</kbd></span><button type="submit" class="builder-submit" aria-label="Build app" disabled><img src="assets/hero/submit.svg" alt=""></button></div><p class="builder-status" role="status"></p>`;
  hero.append(form);
